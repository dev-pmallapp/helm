...
| Timing models | None (FE only) | Atomic / Minor / O3 | Transaction-level | FE / APE / CAE |
...
- **APE (Approximate Emulation)** — per-instruction-class latencies,
...
bring-up, then switch to FS+APE for a region of interest.
