...
fn add_imm_64_basic() {
    ...
    c.x[1] = 100; step(&mut c, &mut m).unwrap(); assert_eq!(c.x[0], 142);
}
#[test]
fn add_imm_32_basic() {
...
fn add_imm_64_shifted() {
    ...
    c.x[1] = 0; step(&mut c, &mut m).unwrap(); assert_eq!(c.x[0], 0x1000);
}
#[test]
fn add_imm_32_wraps() {
    let (mut c, mut m) = cpu_with_code(&[encode_add_imm(0, 0, 1, 1, 0)]);
    c.x[1] = 0xFFFF_FFFF; step(&mut c, &mut m).unwrap(); assert_eq!(c.x[0], 0);
}
#[test]
fn add_imm_64_max() {
    let (mut c, mut m) = cpu_with_code(&[encode_add_imm(1, 0, 0xFFF, 1, 0)]);
    c.x[1] = 0; step(&mut c, &mut m).unwrap(); assert_eq!(c.x[0], 0xFFF);
}
#[test]
fn sub_imm_64_basic() {
    let (mut c, mut m) = cpu_with_code(&[encode_sub_imm(1, 0, 10, 1, 0)]);
    c.x[1] = 50; step(&mut c, &mut m).unwrap(); assert_eq!(c.x[0], 40);
}
#[test]
fn sub_imm_32_basic() {
...
fn sbfm_asr_32() {
    ...
    c.x[1] = 0x8000_0000; step(&mut c, &mut m).unwrap(); assert_eq!(c.x[0], 0xF800_0000);
}
#[test]
fn sbfm_sbfx() {
    let (mut c, mut m) = cpu_with_code(&[encode_sbfm(1, 1, 8, 15, 1, 0)]);
    c.x[1] = 0xFF00; step(&mut c, &mut m).unwrap(); assert_eq!(c.x[0], 0xFFFF_FFFF_FFFF_FFFF);
}
#[test]
fn ubfm_lsl_64() {
...
fn ubfm_uxtb() {
    ...
    c.x[1] = 0x1234_5680; step(&mut c, &mut m).unwrap(); assert_eq!(c.x[0], 0x80);
}
#[test]
fn ubfm_uxth() {
    let (mut c, mut m) = cpu_with_code(&[encode_ubfm(0, 0, 0, 15, 1, 0)]);
    c.x[1] = 0xFFFF_ABCD; step(&mut c, &mut m).unwrap(); assert_eq!(c.x[0], 0xABCD);
}
#[test]
fn ubfm_ubfx() {
...
fn bfm_bfi_32() {
    ...
    c.x[0] = 0xFF; c.x[1] = 0xAB; step(&mut c, &mut m).unwrap(); assert_eq!(c.x[0], 0xABFF);
}
#[test]
fn extr_64_ror() {
    let (mut c, mut m) = cpu_with_code(&[encode_extr(1, 1, 1, 4, 1, 0)]);
    c.x[1] = 0xF; step(&mut c, &mut m).unwrap(); assert_eq!(c.x[0], 0xF000_0000_0000_0000);
}
#[test]
fn extr_32_ror() {
    let (mut c, mut m) = cpu_with_code(&[encode_extr(0, 0, 1, 4, 1, 0)]);
    c.x[1] = 0xF; step(&mut c, &mut m).unwrap(); assert_eq!(c.x[0], 0xF000_0000);
}
#[test]
fn extr_64_concat() {
    let (mut c, mut m) = cpu_with_code(&[encode_extr(1, 1, 2, 8, 1, 0)]);
    c.x[1] = 0xFF; c.x[2] = 0xAB00_0000_0000_0000; step(&mut c, &mut m).unwrap();
    assert_eq!(c.x[0], 0xFFAB_0000_0000_0000);
}
#[test]
fn adr_forward() {
    let (mut c, mut m) = cpu_with_code(&[encode_adr(0x20, 0, 0)]);
    step(&mut c, &mut m).unwrap(); assert_eq!(c.x[0], CODE_BASE + 0x80);
}
#[test]
