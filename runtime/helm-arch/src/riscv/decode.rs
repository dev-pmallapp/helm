...
pub fn decode(raw: u32, pc: u64) -> Result<Instruction, DecodeError> {
    ...
    match opcode {
        ...
        0b0110011 => match (funct7(raw), funct3(raw)) {
            (0b0000000, 0b000) => Ok(ADD {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0100000, 0b000) => Ok(SUB {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000000, 0b001) => Ok(SLL {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000000, 0b010) => Ok(SLT {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000000, 0b011) => Ok(SLTU {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000000, 0b100) => Ok(XOR {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000000, 0b101) => Ok(SRL {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0100000, 0b101) => Ok(SRA {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000000, 0b110) => Ok(OR {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000000, 0b111) => Ok(AND {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            // M extension
            (0b0000001, 0b000) => Ok(MUL {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000001, 0b001) => Ok(MULH {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000001, 0b010) => Ok(MULHSU {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000001, 0b011) => Ok(MULHU {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000001, 0b100) => Ok(DIV {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000001, 0b101) => Ok(DIVU {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000001, 0b110) => Ok(REM {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000001, 0b111) => Ok(REMU {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
        ...
        0b0111011 => match (funct7(raw), funct3(raw)) {
            (0b0000000, 0b000) => Ok(ADDW {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0100000, 0b000) => Ok(SUBW {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000000, 0b001) => Ok(SLLW {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000000, 0b101) => Ok(SRLW {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0100000, 0b101) => Ok(SRAW {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000001, 0b000) => Ok(MULW {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000001, 0b100) => Ok(DIVW {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000001, 0b101) => Ok(DIVUW {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000001, 0b110) => Ok(REMW {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
            (0b0000001, 0b111) => Ok(REMUW {
                rd: rd(raw),
                rs1: rs1(raw),
                rs2: rs2(raw),
            }),
...
fn decode_amo(raw: u32, pc: u64) -> Result<Instruction, DecodeError> {
    ...
    match (f5, f3) {
        (0b00010, 0b010) => Ok(LR_W {
            rd: rd_,
            rs1: r1,
            aq,
            rl,
        }),
        (0b00011, 0b010) => Ok(SC_W {
            rd: rd_,
            rs1: r1,
            rs2: r2,
            aq,
        ...
        (0b00010, 0b011) => Ok(LR_D {
            rd: rd_,
            rs1: r1,
            aq,
            rl,
        }),
        (0b00011, 0b011) => Ok(SC_D {
            rd: rd_,
            rs1: r1,
            rs2: r2,
            aq,
...
fn decode_fp(raw: u32, pc: u64) -> Result<Instruction, DecodeError> {
    ...
    match f25 {
        ...
        0b0100000 if r2 == 1 => Ok(FCVT_S_D {
            rd: rd_,
            rs1: r1,
            rm: rm_,
        }),
        0b0100001 if r2 == 0 => Ok(FCVT_D_S {
            rd: rd_,
            rs1: r1,
            rm: rm_,
        }),
...
pub fn expand_c(c: u16, pc: u64) -> Result<Instruction, DecodeError> {
    ...
    // Helpers
    #[inline(always)]
    fn b(v: u16, hi: u16, lo: u16) -> u16 {
        (v >> lo) & ((1 << (hi - lo + 1)) - 1)
    }
    #[inline(always)]
    fn b1(v: u16, pos: u16) -> u16 {
        (v >> pos) & 1
    }
    ...
    match op {
        ...
        0b00 => match funct3 {
            ...
            0b000 => {
                let rd = cr(b(c, 4, 2));
                let nzuimm = ((b(c, 10, 7) as i64) << 6)
                    | ((b(c, 12, 11) as i64) << 4)
                    | ((b1(c, 6) as i64) << 2)
                    | ((b1(c, 5) as i64) << 3);
                if nzuimm == 0 {
                    return Err(DecodeError::Unknown { raw: c as u32, pc });
                }
                Ok(ADDI {
                    rd,
                    rs1: 2,
        ...
        0b10 => match funct3 {
            ...
            0b100 => {
                ...
                match (bit12, rs1, rs2) {
                    ...
                    (1, r, 0) if r != 0 => Ok(JALR {
                        rd: 1,
                        rs1: r,
                        imm: 0,
                    }), // C.JALR
                    (1, r, s) if r != 0 && s != 0 => Ok(ADD {
                        rd: r,
                        rs1: r,
                        rs2: s,
                    }), // C.ADD
            ...
            0b110 => {
                let rs2 = b(c, 6, 2) as u8;
                let imm = ((b(c, 8, 7) as i64) << 6) | ((b(c, 12, 9) as i64) << 2);
                Ok(SW { rs1: 2, rs2, imm })
            }
            // C.SDSP (RV64C) → sd rs2, offset(x2)
            0b111 => {
                let rs2 = b(c, 6, 2) as u8;
                let imm = ((b(c, 9, 7) as i64) << 6) | ((b(c, 12, 10) as i64) << 3);
                Ok(SD { rs1: 2, rs2, imm })
            }
            _ => Err(DecodeError::Unimplemented),
        },
