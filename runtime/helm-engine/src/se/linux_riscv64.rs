        ...
        match nr {
            ...
            nr::FSTATAT => {
                ...
                if path_ptr != 0 {
                    ...
                    // fstat by fd
                    let host = if dirfd == -100 {
                        self.fds.get(0).unwrap_or(0)
                    } else {
                        self.fds.get(dirfd).unwrap_or(-1)
                    };
                    if host < 0 {
                        return Ok(EBADF);
                    }
            ...
            nr::GETRESUID => {
                if args.a0 != 0 {
                    mem.write(args.a0, 4, 1000, AccessType::Store).ok();
                }
                if args.a1 != 0 {
                    mem.write(args.a1, 4, 1000, AccessType::Store).ok();
                }
                if args.a2 != 0 {
                    mem.write(args.a2, 4, 1000, AccessType::Store).ok();
                }
                Ok(0)
            }
            nr::GETRESGID => {
            ...
            _ => {
                log::warn!(
                    "riscv64 unimplemented syscall {nr} (a0={:#x}, a1={:#x})",
                    args.a0,
                    args.a1
                );
                Ok(ENOSYS)
...
impl super::SyscallHandler for LinuxRiscv64SyscallHandler {
    fn handle(
        ...
        nr: u64,
        args: SyscallArgs,
        mem: &mut dyn helm_core::MemInterface,
    ) -> Result<i64, HartException> {
...
fn write_stat(mem: &mut dyn MemInterface, ptr: u64, st: &libc::stat) {
    ...
    mem.write(ptr + 48, 8, st.st_size as u64, AccessType::Store)
        .ok();
    mem.write(ptr + 56, 8, st.st_blksize as u64, AccessType::Store)
        .ok();
