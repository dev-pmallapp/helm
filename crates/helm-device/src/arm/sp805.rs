...
impl Sp805 {
    ...
    fn handle_read(&self, offset: u64) -> u32 {
        match offset {
            ...
            0xFE0 => 0x05, 0xFE4 => 0x18, 0xFE8 => 0x14, 0xFEC => 0x00,
    ...
    fn handle_write(&mut self, offset: u64, value: u32) {
        if self.locked && offset != WDOGLOCK { return; }
        match offset {
            WDOGLOAD => { self.load = value; self.value = value; }
            WDOGCONTROL => self.control = value & 3,
            WDOGINTCLR => { self.raw_irq = false; self.value = self.load; }
...
impl Device for Sp805 {
    ...
    fn reset(&mut self) -> HelmResult<()> {
        self.load = 0xFFFF_FFFF; self.value = 0xFFFF_FFFF;
        self.control = 0; self.raw_irq = false; self.locked = false;
        Ok(())
    }

    fn tick(&mut self, cycles: u64) -> HelmResult<Vec<DeviceEvent>> {
        if self.control & CTRL_INTEN == 0 { return Ok(vec![]); }
