...
impl Pl061 {
    pub fn new(name: impl Into<String>) -> Self {
        ...
        Self {
            ...
            dev_name: n,
            data: 0, dir: 0, is: 0, ibe: 0, iev: 0, ie: 0, ris: 0, afsel: 0, old_data: 0,
        }
    }
    ...
    pub fn set_input(&mut self, pin: u8, high: bool) {
        if pin >= 8 { return; }
        if self.dir & (1 << pin) != 0 { return; } // output pin, ignore
        if high { self.data |= 1 << pin; }
        else { self.data &= !(1 << pin); }
    }
    ...
    fn handle_read(&self, offset: u64) -> u32 {
        match offset {
            ...
            // PrimeCell ID (PL061)
            0xFE0 => 0x61, 0xFE4 => 0x10, 0xFE8 => 0x04, 0xFEC => 0x00,
...
impl Device for Pl061 {
    ...
    fn reset(&mut self) -> HelmResult<()> {
        self.data = 0; self.dir = 0; self.is = 0; self.ibe = 0;
        self.iev = 0; self.ie = 0; self.ris = 0; self.afsel = 0;
        self.old_data = 0;
        Ok(())
    }
    ...
    fn tick(&mut self, _cycles: u64) -> HelmResult<Vec<DeviceEvent>> {
        ...
        for i in 0..8u8 {
            ...
            let triggered = if self.is & bit != 0 {
                // Level-sensitive
                if self.iev & bit != 0 { self.data & bit != 0 } else { self.data & bit == 0 }
            } else {
                // Edge-sensitive
                if self.ibe & bit != 0 {
                    true // both edges
            ...
            if triggered { self.ris |= bit; }
        }
        self.old_data = self.data;

        if self.ris & self.ie != 0 {
            Ok(vec![DeviceEvent::Irq { line: 0, assert: true }])
        } else {
            Ok(vec![])
        }
    }
