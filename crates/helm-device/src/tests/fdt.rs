...
fn empty_dtb_has_valid_header() {
    ...
    assert_eq!(
        u32::from_be_bytes([blob[4], blob[5], blob[6], blob[7]]) as usize,
        blob.len()
    );
}
...
fn infer_virt_kernel_no_dtb_generates() {
    assert_eq!(
        DtbPolicy::infer(&ctx(true, true, false, false, false, false)),
        DtbPolicy::Generate
    );
}
...
fn infer_virt_kernel_with_dtb_passthrough() {
    assert_eq!(
        DtbPolicy::infer(&ctx(true, true, false, false, true, false)),
...
fn infer_virt_kernel_dtb_plus_extras_patches() {
    assert_eq!(
        DtbPolicy::infer(&ctx(true, true, false, false, true, true)),
        DtbPolicy::Patch
    );
}
...
fn infer_bios_means_no_dtb() {
    assert_eq!(
        DtbPolicy::infer(&ctx(true, false, true, false, true, true)),
        DtbPolicy::None
    );
}
...
fn infer_drive_only_no_kernel_no_dtb() {
    assert_eq!(
        DtbPolicy::infer(&ctx(true, false, false, true, false, false)),
        DtbPolicy::None
    );
}
...
fn infer_drive_only_with_dtb_passthrough() {
    assert_eq!(
        DtbPolicy::infer(&ctx(true, false, false, true, true, false)),
        DtbPolicy::Passthrough
    );
}
...
fn infer_non_dtb_platform_always_none() {
    assert_eq!(
        DtbPolicy::infer(&ctx(false, true, false, false, false, false)),
        DtbPolicy::None
    );
}
...
fn resolve_generate_for_kernel_boot() {
    ...
    match resolve_dtb(&p, &cfg, None, &c) {
        ResolvedDtb::Blob(b) => {
            assert_eq!(u32::from_be_bytes([b[0], b[1], b[2], b[3]]), 0xD00DFEED)
        }
