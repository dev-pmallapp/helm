...
fn pcie_dev_cap_max_payload_256b() {
    ...
    assert_eq!(dev_cap & 0x7, 0x1, "max payload should be 256B (encoding 1)");
}
...
fn msix_vector_clone() {
    let v = MsixVector { addr_lo: 0xFEE0_0000, addr_hi: 0, data: 0x41, masked: false, pending: false };
    let v2 = v.clone();
...
fn pm_pme_status_w1c() {
    ...
    cap.write(0x04, 1 << 15);
    let pm_csr = cap.read(0x04);
    assert_eq!((pm_csr >> 15) & 1, 0, "PME_Status should remain 0 after W1C of already-0 bit");
}
...
fn aer_correctable_status_w1c() {
    ...
    assert_eq!(cap.read(0x10) & 0x0001, 0, "correctable status cleared by W1C");
}
...
fn acs_cap_register_readonly() {
    ...
    let dword = cap.read(0x04);
    assert_eq!(dword & 0xFFFF, 0x001F, "acs_cap should remain 0x001F (read-only)");
}
