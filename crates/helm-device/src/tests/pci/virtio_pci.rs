...
fn has_five_virtio_vendor_specific_caps() {
    let t = make_rng_pci();
    let count = t.capabilities().iter().filter(|c| c.cap_id() == 0x09).count();
...
fn device_feature_high_select_1() {
    ...
    assert!(features & 1 != 0, "VERSION_1 should be set in high feature word");
}
...
fn blk_device_config_capacity_readable() {
    ...
    assert_eq!(capacity, 8, "capacity should be 8 sectors for a 4096-byte disk");
}
