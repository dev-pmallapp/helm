...
fn bus_config_write_then_read_command_reg() {
    ...
    assert_eq!(val & 0x0002, 0x0002, "memory space enable bit should be set");
}
...
fn bridge_two_devices_non_overlapping_bars() {
    ...
    assert!(no_overlap, "BARs must not overlap: [{:#x}..{:#x}) and [{:#x}..{:#x})", a1, end1, a2, end2);
}
...
fn bridge_mmio_read_write_via_transact() {
    ...
    assert_eq!(rtxn.data_u64(), write_val, "BAR MMIO round-trip via transact");
}
...
fn bridge_transact_adds_stall_cycle() {
    ...
    assert!(txn.stall_cycles >= 1, "transact must add at least 1 stall cycle");
}
