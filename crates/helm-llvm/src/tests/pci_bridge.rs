...
fn control_reset_returns_to_idle() {
    ...
    assert_eq!(a.bar_read(0, 0x00, 4), 0, "STATUS should be 0 (idle) after reset");
}
...
fn control_abort_returns_to_idle() {
    ...
    assert_eq!(a.bar_read(0, 0x00, 4), 0, "STATUS should be 0 (idle) after abort");
}
...
fn nonexistent_file_status_is_error() {
    ...
    assert_eq!(a.bar_read(0, 0x00, 4), 3, "STATUS should be 3 (error) for nonexistent file");
}
...
fn pci_reset_returns_to_idle() {
    ...
    assert_eq!(a.bar_read(0, 0x00, 4), 2);
    PciFunction::reset(&mut a);
    assert_eq!(a.bar_read(0, 0x00, 4), 0, "STATUS should be 0 after PciFunction::reset");
}
