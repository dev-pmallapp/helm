...
fn write_u64(mem: &mut AddressSpace, addr: u64, val: u64) {
    mem.write(addr, &val.to_le_bytes()).unwrap();
}
fn write_u32(mem: &mut AddressSpace, addr: u64, val: u32) {
    mem.write(addr, &val.to_le_bytes()).unwrap();
}
fn write_u16(mem: &mut AddressSpace, addr: u64, val: u16) {
    mem.write(addr, &val.to_le_bytes()).unwrap();
}
fn write_u8(mem: &mut AddressSpace, addr: u64, val: u8) {
    mem.write(addr, &[val]).unwrap();
}
fn read_u64(mem: &mut AddressSpace, addr: u64) -> u64 {
    let mut b = [0u8; 8];
    mem.read(addr, &mut b).unwrap();
    u64::from_le_bytes(b)
}
fn read_u32(mem: &mut AddressSpace, addr: u64) -> u32 {
    let mut b = [0u8; 4];
    mem.read(addr, &mut b).unwrap();
    u32::from_le_bytes(b)
}
fn read_u16(mem: &mut AddressSpace, addr: u64) -> u16 {
    let mut b = [0u8; 2];
    mem.read(addr, &mut b).unwrap();
    u16::from_le_bytes(b)
}
fn read_u8(mem: &mut AddressSpace, addr: u64) -> u8 {
    let mut b = [0u8; 1];
    mem.read(addr, &mut b).unwrap();
    b[0]
}
...
fn str_ldr_x64_roundtrip() {
    ...
    c.set_xn(0, 0xDEAD_BEEF_CAFE_1234);
    c.set_xn(2, DATA);
    c.step(&mut m).unwrap();
    c.step(&mut m).unwrap();
...
fn stp_ldp_x_roundtrip() {
    ...
    c.step(&mut m).unwrap();
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(4), 0xAAAA);
    assert_eq!(c.xn(3), 0xBBBB);
}
#[test]
...
fn ldp_x_post_index() {
    ...
    c.set_xn(2, DATA);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(4), 0x1111);
    assert_eq!(c.xn(3), 0x2222);
...
fn ldxr_stxr_success() {
    ...
    assert_eq!(c.xn(0), 42);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(2), 0, "STXR success");
