...
fn exception_entry_saves_state() {
    ...
    assert_eq!(c.regs.elr_el1, BASE, "ELR should point to SVC instruction");
...
fn exception_roundtrip() {
    ...
    assert_eq!(cpu.regs.elr_el1, svc_addr);

    // Step 2: ERET → returns to EL0
    cpu.step(&mut mem).unwrap();
    assert_eq!(cpu.regs.current_el, 0);
    assert_eq!(cpu.regs.pc, svc_addr, "ERET returns to SVC address");
