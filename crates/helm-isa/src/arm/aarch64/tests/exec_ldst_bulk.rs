...
fn ldr_x(i: u32, rn: u32, rt: u32) -> u32  { str_uoff(0b11, 0b01, i, rn, rt) }
fn str_x(i: u32, rn: u32, rt: u32) -> u32  { str_uoff(0b11, 0b00, i, rn, rt) }
fn ldr_w(i: u32, rn: u32, rt: u32) -> u32  { str_uoff(0b10, 0b01, i, rn, rt) }
fn str_w(i: u32, rn: u32, rt: u32) -> u32  { str_uoff(0b10, 0b00, i, rn, rt) }
fn ldrh(i: u32, rn: u32, rt: u32) -> u32   { str_uoff(0b01, 0b01, i, rn, rt) }
fn strh(i: u32, rn: u32, rt: u32) -> u32   { str_uoff(0b01, 0b00, i, rn, rt) }
fn ldrb(i: u32, rn: u32, rt: u32) -> u32   { str_uoff(0b00, 0b01, i, rn, rt) }
fn strb(i: u32, rn: u32, rt: u32) -> u32   { str_uoff(0b00, 0b00, i, rn, rt) }
fn ldrsw(i: u32, rn: u32, rt: u32) -> u32  { str_uoff(0b10, 0b10, i, rn, rt) }
fn ldrsb_w(i: u32, rn: u32, rt: u32) -> u32{ str_uoff(0b00, 0b11, i, rn, rt) }
fn ldrsb_x(i: u32, rn: u32, rt: u32) -> u32{ str_uoff(0b00, 0b10, i, rn, rt) }
fn ldrsh_w(i: u32, rn: u32, rt: u32) -> u32{ str_uoff(0b01, 0b11, i, rn, rt) }
fn ldrsh_x(i: u32, rn: u32, rt: u32) -> u32{ str_uoff(0b01, 0b10, i, rn, rt) }

// STP/LDP encodings
fn stp_x(imm7: i32, rt2: u32, rn: u32, rt: u32) -> u32 { (0b10_101_0_0_10_0u32 << 22) | (((imm7 as u32) & 0x7F) << 15) | (rt2 << 10) | (rn << 5) | rt }
fn ldp_x(imm7: i32, rt2: u32, rn: u32, rt: u32) -> u32 { (0b10_101_0_0_10_1u32 << 22) | (((imm7 as u32) & 0x7F) << 15) | (rt2 << 10) | (rn << 5) | rt }
fn stp_x_pre(imm7: i32, rt2: u32, rn: u32, rt: u32) -> u32 { (0b10_101_0_0_11_0u32 << 22) | (((imm7 as u32) & 0x7F) << 15) | (rt2 << 10) | (rn << 5) | rt }
fn ldp_x_pre(imm7: i32, rt2: u32, rn: u32, rt: u32) -> u32 { (0b10_101_0_0_11_1u32 << 22) | (((imm7 as u32) & 0x7F) << 15) | (rt2 << 10) | (rn << 5) | rt }
fn stp_x_post(imm7: i32, rt2: u32, rn: u32, rt: u32) -> u32 { (0b10_101_0_0_01_0u32 << 22) | (((imm7 as u32) & 0x7F) << 15) | (rt2 << 10) | (rn << 5) | rt }
fn ldp_x_post(imm7: i32, rt2: u32, rn: u32, rt: u32) -> u32 { (0b10_101_0_0_01_1u32 << 22) | (((imm7 as u32) & 0x7F) << 15) | (rt2 << 10) | (rn << 5) | rt }
fn stp_w(imm7: i32, rt2: u32, rn: u32, rt: u32) -> u32 { (0b00_101_0_0_10_0u32 << 22) | (((imm7 as u32) & 0x7F) << 15) | (rt2 << 10) | (rn << 5) | rt }
fn ldp_w(imm7: i32, rt2: u32, rn: u32, rt: u32) -> u32 { (0b00_101_0_0_10_1u32 << 22) | (((imm7 as u32) & 0x7F) << 15) | (rt2 << 10) | (rn << 5) | rt }
...
macro_rules! test_str_ldr {
    ($name:ident, $str_fn:ident, $ldr_fn:ident, $write_val:expr, $expected:expr, $offset:expr) => {
        #[test] fn $name() {
            ...
            c.set_xn(0, $write_val); c.set_xn(3, D);
...
macro_rules! test_ldrs {
    ($name:ident, $ldr_fn:ident, $wr_fn:ident, $mem_val:expr, $expected:expr) => {
        #[test] fn $name() {
            ...
            $wr_fn(&mut m, D, $mem_val); c.set_xn(3, D);
            c.step(&mut m).unwrap();
...
macro_rules! test_stp_ldp {
    ($name:ident, $stp:ident, $ldp:ident, $off:expr, $a:expr, $b:expr) => {
        #[test] fn $name() {
            ...
            c.set_xn(0, $a); c.set_xn(1, $b); c.set_xn(3, D + 0x100);
            c.step(&mut m).unwrap(); c.step(&mut m).unwrap();
            assert_eq!(c.xn(4), $a); assert_eq!(c.xn(5), $b);
        }
    };
}
...
// ===================================================================
test_str_ldr!(str_ldr_x_off0,   str_x, ldr_x, 0xDEAD_BEEF_CAFE_1234u64, 0xDEAD_BEEF_CAFE_1234u64, 0);
...
test_str_ldr!(str_ldr_x_off100, str_x, ldr_x, 0xAABBCCDDu64, 0xAABBCCDDu64, 100);

// ===================================================================
//  STR / LDR — W (32-bit) at various offsets
// ===================================================================
test_str_ldr!(str_ldr_w_off0,  str_w, ldr_w, 0x1_FFFF_FFFFu64, 0xFFFF_FFFFu64, 0);
test_str_ldr!(str_ldr_w_off1,  str_w, ldr_w, 0x12345678u64, 0x12345678u64, 1);
test_str_ldr!(str_ldr_w_off5,  str_w, ldr_w, 0u64, 0u64, 5);
test_str_ldr!(str_ldr_w_off50, str_w, ldr_w, 0xDEADBEEFu64, 0xDEADBEEFu64, 50);
...
test_ldrs!(ldrsw_neg_msb,    ldrsw, wr32, 0x8000_0000u32, 0xFFFF_FFFF_8000_0000u64);
test_ldrs!(ldrsw_neg_1,      ldrsw, wr32, 0xFFFF_FFFFu32, 0xFFFF_FFFF_FFFF_FFFFu64);
test_ldrs!(ldrsw_zero,       ldrsw, wr32, 0u32, 0u64);
test_ldrs!(ldrsw_small_neg,  ldrsw, wr32, 0xFFFF_FFF0u32, 0xFFFF_FFFF_FFFF_FFF0u64);
...
test_ldrs!(ldrsh_x_neg,      ldrsh_x, wr16, 0x8000u16, 0xFFFF_FFFF_FFFF_8000u64);
test_ldrs!(ldrsh_x_neg1,     ldrsh_x, wr16, 0xFFFFu16, 0xFFFF_FFFF_FFFF_FFFFu64);
...
#[test] fn ldp_pre_pos2() {
    ...
    wr64(&mut m, D + 0x110, 0x1111); wr64(&mut m, D + 0x118, 0x2222);
    c.set_xn(3, D + 0x100);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(3), D + 0x110, "pre-index increments");
    assert_eq!(c.xn(0), 0x1111); assert_eq!(c.xn(1), 0x2222);
}
...
#[test] fn ldp_post_neg2() {
    let (mut c, mut m) = cpu_exec(&[ldp_x_post(-2, 1, 3, 0)]);
    wr64(&mut m, D + 0x100, 0x5555); wr64(&mut m, D + 0x108, 0x6666);
    c.set_xn(3, D + 0x100);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0x5555); assert_eq!(c.xn(1), 0x6666);
...
test_str_verify!(str_w_verify_max,  str_w, rd32, 0xFFFF_FFFFu64, 0xFFFF_FFFFu32);
test_str_verify!(str_w_verify_trunc,str_w, rd32, 0x1_2345_6789u64, 0x2345_6789u32);
...
#[test] fn ldr_x_via_sp() {
    let (mut c, mut m) = cpu_exec(&[str_x(0, 31, 0), ldr_x(0, 31, 1)]);
    c.set_xn(0, 0xCAFE); c.step(&mut m).unwrap(); c.step(&mut m).unwrap();
    assert_eq!(c.xn(1), 0xCAFE);
}
#[test] fn str_w_via_sp() {
    let (mut c, mut m) = cpu_exec(&[str_w(0, 31, 0), ldr_w(0, 31, 1)]);
    c.set_xn(0, 0xBEEF); c.step(&mut m).unwrap(); c.step(&mut m).unwrap();
    assert_eq!(c.xn(1), 0xBEEF);
}
#[test] fn strb_via_sp() {
    let (mut c, mut m) = cpu_exec(&[strb(0, 31, 0), ldrb(0, 31, 1)]);
    c.set_xn(0, 0xAB); c.step(&mut m).unwrap(); c.step(&mut m).unwrap();
    assert_eq!(c.xn(1), 0xAB);
}
#[test] fn strh_via_sp() {
    let (mut c, mut m) = cpu_exec(&[strh(0, 31, 0), ldrh(0, 31, 1)]);
    c.set_xn(0, 0x1234); c.step(&mut m).unwrap(); c.step(&mut m).unwrap();
...
#[test] fn str_ldr_x_multiple_regs() {
    let (mut c, mut m) = cpu_exec(&[
        str_x(0, 10, 0), str_x(1, 10, 1), str_x(2, 10, 2),
        ldr_x(0, 10, 3), ldr_x(1, 10, 4), ldr_x(2, 10, 5),
    ]);
    c.set_xn(0, 100); c.set_xn(1, 200); c.set_xn(2, 300); c.set_xn(10, D);
    for _ in 0..6 { c.step(&mut m).unwrap(); }
    assert_eq!(c.xn(3), 100); assert_eq!(c.xn(4), 200); assert_eq!(c.xn(5), 300);
}

#[test] fn stp_ldp_chain_4_regs() {
    let (mut c, mut m) = cpu_exec(&[
        stp_x(0, 1, 10, 0), stp_x(2, 3, 10, 2),
        ldp_x(0, 5, 10, 4), ldp_x(2, 7, 10, 6),
    ]);
    c.set_xn(0, 10); c.set_xn(1, 20); c.set_xn(2, 30); c.set_xn(3, 40); c.set_xn(10, D);
    for _ in 0..4 { c.step(&mut m).unwrap(); }
    assert_eq!(c.xn(4), 10); assert_eq!(c.xn(5), 20);
    assert_eq!(c.xn(6), 30); assert_eq!(c.xn(7), 40);
}

// ===================================================================
//  STR/LDR X at many more offsets
// ===================================================================
test_str_ldr!(str_ldr_x_off3,   str_x, ldr_x, 0x1234_5678u64, 0x1234_5678u64, 3);
test_str_ldr!(str_ldr_x_off4,   str_x, ldr_x, 0x1234_5678u64, 0x1234_5678u64, 4);
test_str_ldr!(str_ldr_x_off5,   str_x, ldr_x, 0x1234_5678u64, 0x1234_5678u64, 5);
test_str_ldr!(str_ldr_x_off8,   str_x, ldr_x, 0x1234_5678u64, 0x1234_5678u64, 8);
test_str_ldr!(str_ldr_x_off16,  str_x, ldr_x, 0x1234_5678u64, 0x1234_5678u64, 16);
test_str_ldr!(str_ldr_x_off32,  str_x, ldr_x, 0x1234_5678u64, 0x1234_5678u64, 32);
test_str_ldr!(str_ldr_x_off64,  str_x, ldr_x, 0x1234_5678u64, 0x1234_5678u64, 64);
test_str_ldr!(str_ldr_x_off128, str_x, ldr_x, 0x1234_5678u64, 0x1234_5678u64, 128);
test_str_ldr!(str_ldr_x_off255, str_x, ldr_x, 0x1234_5678u64, 0x1234_5678u64, 255);
...
test_ldrs!(ldrsh_x_32768,     ldrsh_x, wr16, 0x8000u16, 0xFFFF_FFFF_FFFF_8000u64);
