...
// Condition codes
const EQ: u32 = 0; const NE: u32 = 1;
const CS: u32 = 2; const CC: u32 = 3;
const MI: u32 = 4; const PL: u32 = 5;
const VS: u32 = 6; const VC: u32 = 7;
const HI: u32 = 8; const LS: u32 = 9;
const GE: u32 = 10; const LT: u32 = 11;
const GT: u32 = 12; const LE: u32 = 13;
const AL: u32 = 14;
...
#[test] fn b_forward_4() { let (mut c, mut m) = cpu_with_code(&[encode_b(1), NOP]); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 4); }
#[test] fn b_forward_8() { let (mut c, mut m) = cpu_with_code(&[encode_b(2), NOP, NOP]); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 8); }
#[test] fn b_forward_1024() { let (mut c, mut m) = cpu_with_code(&[encode_b(256)]); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 1024); }
#[test] fn bl_sets_lr() {
...
#[test] fn cbz_64_taken()  { let (mut c, mut m) = cpu_with_code(&[encode_cbz(1, 2, 0), NOP, NOP]); c.set_xn(0, 0); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 8); }
#[test] fn cbz_64_not()    { let (mut c, mut m) = cpu_with_code(&[encode_cbz(1, 2, 0), NOP, NOP]); c.set_xn(0, 1); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 4); }
#[test] fn cbz_32_taken()  { let (mut c, mut m) = cpu_with_code(&[encode_cbz(0, 2, 0), NOP, NOP]); c.set_xn(0, 0x1_0000_0000); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 8, "32-bit CBZ ignores upper bits"); }
#[test] fn cbz_32_not()    { let (mut c, mut m) = cpu_with_code(&[encode_cbz(0, 2, 0), NOP, NOP]); c.set_xn(0, 1); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 4); }
#[test] fn cbnz_64_taken() { let (mut c, mut m) = cpu_with_code(&[encode_cbnz(1, 2, 0), NOP, NOP]); c.set_xn(0, 42); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 8); }
#[test] fn cbnz_64_not()   { let (mut c, mut m) = cpu_with_code(&[encode_cbnz(1, 2, 0), NOP, NOP]); c.set_xn(0, 0); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 4); }
#[test] fn cbnz_32_taken() { let (mut c, mut m) = cpu_with_code(&[encode_cbnz(0, 2, 0), NOP, NOP]); c.set_xn(0, 0xFF); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 8); }

// ===================================================================
//  TBZ / TBNZ
// ===================================================================

#[test] fn tbz_bit0_taken()  { let (mut c, mut m) = cpu_with_code(&[encode_tbz(0, 0, 2, 0), NOP, NOP]); c.set_xn(0, 0xFFFE); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 8, "bit 0 clear"); }
#[test] fn tbz_bit0_not()    { let (mut c, mut m) = cpu_with_code(&[encode_tbz(0, 0, 2, 0), NOP, NOP]); c.set_xn(0, 0xFFFF); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 4, "bit 0 set"); }
#[test] fn tbz_bit31_taken() { let (mut c, mut m) = cpu_with_code(&[encode_tbz(0, 31, 2, 0), NOP, NOP]); c.set_xn(0, 0x7FFF_FFFF); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 8); }
#[test] fn tbz_bit31_not()   { let (mut c, mut m) = cpu_with_code(&[encode_tbz(0, 31, 2, 0), NOP, NOP]); c.set_xn(0, 0x8000_0000); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 4); }
#[test] fn tbz_bit63_taken() { let (mut c, mut m) = cpu_with_code(&[encode_tbz(1, 31, 2, 0), NOP, NOP]); c.set_xn(0, 0x7FFF_FFFF_FFFF_FFFF); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 8); }
#[test] fn tbz_bit63_not()   { let (mut c, mut m) = cpu_with_code(&[encode_tbz(1, 31, 2, 0), NOP, NOP]); c.set_xn(0, 0x8000_0000_0000_0000); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 4); }
#[test] fn tbnz_bit0_taken() { let (mut c, mut m) = cpu_with_code(&[encode_tbnz(0, 0, 2, 0), NOP, NOP]); c.set_xn(0, 1); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 8); }
#[test] fn tbnz_bit0_not()   { let (mut c, mut m) = cpu_with_code(&[encode_tbnz(0, 0, 2, 0), NOP, NOP]); c.set_xn(0, 0); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 4); }
#[test] fn tbnz_bit16()      { let (mut c, mut m) = cpu_with_code(&[encode_tbnz(0, 16, 2, 0), NOP, NOP]); c.set_xn(0, 1 << 16); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, BASE + 8); }

// ===================================================================
//  BR / BLR / RET
// ===================================================================

#[test] fn br_to_addr()  { let (mut c, mut m) = cpu_with_code(&[encode_br(1)]); c.set_xn(1, 0x50_0000); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, 0x50_0000); }
#[test] fn blr_sets_lr() {
...
#[test] fn ret_default() { let (mut c, mut m) = cpu_with_code(&[encode_ret(30)]); c.set_xn(30, 0x60_0000); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, 0x60_0000); }
#[test] fn ret_custom_reg() { let (mut c, mut m) = cpu_with_code(&[encode_ret(5)]); c.set_xn(5, 0x70_0000); c.step(&mut m).unwrap(); assert_eq!(c.regs.pc, 0x70_0000); }
