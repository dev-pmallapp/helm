...
use helm_jit::a64_emitter::{A64TcgEmitter, TranslateAction};
use helm_jit::block::TcgBlock;
use helm_jit::interp::{InterpExit, TcgInterp, NUM_REGS, REG_NZCV, REG_PC, REG_SP};
use helm_jit::TcgContext;
...
pub(crate) fn exec_tcg(
    ...
    if let Some((block, _)) = cache.get(&pc) {
        ...
        // For SVC exceptions, extract ELR/SPSR from the regs array
        // BEFORE array_to_regs (which only copies GP regs in SE mode).
        let svc_info = match result.exit {
            InterpExit::Exception { class: 0x15, .. } => {
                use helm_jit::interp::{REG_ELR_EL1, REG_SPSR_EL1};
                let elr = regs[REG_ELR_EL1 as usize];
                let spsr = regs[REG_SPSR_EL1 as usize] as u32;
                let nr = regs[8]; // X8 = syscall number
                Some((elr, spsr, nr))
            }
            _ => None,
        };
        ...
        *insn_count += n;
        ...
        match result.exit {
            ...
            InterpExit::Exit => {}
            InterpExit::Exception { class: 0x15, .. } => {
                // SVC in a TCG block — extract saved state from regs array
                // and route through the syscall handler.
                let (elr, spsr, nr) = svc_info.unwrap();
                // Restore pre-exception PSTATE from SPSR.
                cpu.regs.nzcv = spsr & 0xF000_0000;
                cpu.regs.daif = spsr & 0x3C0;
                cpu.regs.current_el = ((spsr >> 2) & 3) as u8;
                cpu.regs.sp_sel = (spsr & 1) as u8;
                // Set PC to SVC address; handle_sc will advance by 4.
                let svc_pc = elr.wrapping_sub(4);
                cpu.regs.pc = svc_pc;
                handle_sc(
                    cpu,
                    mem,
            ...
            InterpExit::Wfi | InterpExit::Exception { .. } | InterpExit::ExceptionReturn => {
                // Other FS-mode events; in SE mode, treat as no-op.
            }
        }
    } else {
