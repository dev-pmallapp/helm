...
use helm_jit::interp::{sysreg_idx as tcg_sysreg_idx, MMU_DIRTY_IDX};
use helm_jit::interp::{InterpExit, TcgInterp, NUM_REGS, REG_NZCV, REG_PC, REG_SP, REG_TPIDR_EL0};
use helm_jit::ir::TcgOp;
use helm_jit::threaded::{self, CompiledBlock};
...
pub struct FsSession {
    ...
    jit_engine: Option<helm_jit::jit::JitEngine>,
    ...
    pub fn from_platform(
        ...
        Ok(Self {
            ...
            jit_engine: if opts.backend == "jit" {
                unsafe {
                    helm_jit::jit::set_translate_va(jit_translate_va);
                }
                unsafe {
                    helm_jit::jit::set_tlbi_cb(jit_tlbi);
                }
                Some(helm_jit::jit::JitEngine::new())
            } else {
                None
            },
            jit_cache: {
                ...
                if self.jit_cache[jidx].as_ref().map_or(false, |e| e.pc == pc) {
                    ...
                    let sysregs_slice = unsafe {
                        std::slice::from_raw_parts_mut(
                            sysregs_raw,
                            helm_jit::interp::SYSREG_FILE_SIZE,
                        )
                    };
...
fn sync_mmu_to_cpu(cpu: &mut Aarch64Cpu, regs: &[u64; NUM_REGS], sysregs: &[u64]) {
    use helm_isa::arm::aarch64::sysreg;
    use helm_jit::interp::sysreg_idx;
