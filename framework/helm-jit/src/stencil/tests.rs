...
fn stencil_vs_interp_add_imm_w() {
    let mut init = InitState::default();
    init.x[1] = 0xDEAD_BEEF_0000_0064; // upper 32 set to verify zero-extension
    // ADD W0, W1, #1  (sf=0, op=0, S=0, shift=0, imm12=1, Rn=1, Rd=0)
    // Encoding: 0001_0001 0000_0000 0000_0100 0010_0000 = 0x11000420
    assert_stencil_matches_interpreter(0x11000420, 0x1000, &init, "ADD W0, W1, #1");
}
...
fn stencil_vs_interp_sub_imm_w() {
    let mut init = InitState::default();
    init.x[1] = 100;
    // SUB W0, W1, #10  (sf=0, op=1, S=0, shift=0, imm12=10, Rn=1, Rd=0)
    // Encoding: 0101_0001 0000_0000 0010_1000 0010_0000 = 0x51002820
    assert_stencil_matches_interpreter(0x51002820, 0x1000, &init, "SUB W0, W1, #10");
}
...
fn stencil_vs_interp_orr_reg_w() {
    ...
    init.x[1] = 0xAA;
    init.x[2] = 0x55;
    // ORR W0, W1, W2  (sf=0, opc=01, shift=0, N=0, Rm=2, imm6=0, Rn=1, Rd=0)
    // Encoding: 0010_1010 0000_0010 0000_0000 0010_0000 = 0x2a020020
    assert_stencil_matches_interpreter(0x2a020020, 0x1000, &init, "ORR W0, W1, W2");
}

#[test]
fn stencil_vs_interp_movz_w() {
    // MOVZ W0, #0x1234  (sf=0, opc=10, hw=0, imm16=0x1234, Rd=0)
    // Encoding: 0101_0010 1000_0010 0100_0110 1000_0000 = 0x52824680
    assert_stencil_matches_interpreter(
        0x52824680,
        0x1000,
        &InitState::default(),
...
#[test]
fn stencil_vs_interp_w_register_zeroes_upper_32() {
    let mut init = InitState::default();
    init.x[0] = 0xFFFF_FFFF_FFFF_FFFF; // all bits set
    init.x[1] = 1;
    // ADD W0, W1, #0  → W0 = 1, X0 must be 0x0000_0000_0000_0001
    // Encoding: 0001_0001 0000_0000 0000_0000 0010_0000 = 0x11000020
    assert_stencil_matches_interpreter(
        0x11000020,
        0x1000,
        &init,
        "ADD W0, W1, #0 (upper 32 cleared)",
    );
}

#[test]
fn stencil_vs_interp_add_reg_w() {
    let mut init = InitState::default();
    init.x[1] = 10;
    init.x[2] = 20;
    // ADD W0, W1, W2  (sf=0)
    // Encoding: 0000_1011 0000_0010 0000_0000 0010_0000 = 0x0b020020
    assert_stencil_matches_interpreter(0x0b020020, 0x1000, &init, "ADD W0, W1, W2");
}

#[test]
fn stencil_vs_interp_sub_reg_w() {
    let mut init = InitState::default();
    init.x[1] = 100;
    init.x[2] = 30;
    // SUB W0, W1, W2  (sf=0)
    // Encoding: 0100_1011 0000_0010 0000_0000 0010_0000 = 0x4b020020
    assert_stencil_matches_interpreter(0x4b020020, 0x1000, &init, "SUB W0, W1, W2");
}

#[test]
fn stencil_vs_interp_madd_w() {
    let mut init = InitState::default();
    init.x[1] = 6;
    init.x[2] = 7;
    init.x[3] = 0;
    // MADD W0, W1, W2, W3  (sf=0)
    // Encoding: 0001_1011 0000_0010 0000_1100 0010_0000 = 0x1b020c20
    assert_stencil_matches_interpreter(0x1b020c20, 0x1000, &init, "MADD W0, W1, W2, W3");
}

#[test]
fn stencil_vs_interp_movn_w() {
    // MOVN W0, #1  (sf=0)
    // Encoding: 0001_0010 1000_0000 0000_0000 0010_0000 = 0x12800020
    assert_stencil_matches_interpreter(
        0x12800020,
        0x400100,
        &InitState::default(),
...
#[test]
fn stencil_vs_interp_eor_reg_w() {
    let mut init = InitState::default();
    init.x[0] = 0xFF;
    init.x[1] = 0x0F;
    // EOR W0, W0, W1  (sf=0)
    // Encoding: 0100_1010 0000_0001 0000_0000 0000_0000 = 0x4a010000
    assert_stencil_matches_interpreter(0x4a010000, 0x1000, &init, "EOR W0, W0, W1");
}
