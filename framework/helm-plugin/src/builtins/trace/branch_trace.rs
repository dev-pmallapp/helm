...
impl HelmPlugin for BranchTrace {
    ...
    fn install(&mut self, reg: &mut HelmPluginRegistry, args: &HelmPluginArgs) {
        ...
        reg.on_branch(Box::new(move |_vcpu_idx, info| {
            ...
            let rec = guard.entry(info.pc).or_insert(BranchRecord {
                taken: 0,
                not_taken: 0,
            });
            if info.taken {
    ...
    fn atexit(&mut self) {
        ...
        eprintln!(
            "[branch-trace] {} unique branch PCs, {} total branches",
            guard.len(),
            total
        );
        ...
        for (pc, rec) in entries.iter().take(self.top_n) {
            ...
            let pct = if total > 0 {
                (rec.taken as f64 / total as f64) * 100.0
            } else {
                0.0
            };
            eprintln!(
                "[branch-trace]   {:#018x}: {} taken / {} not-taken ({:.1}% taken)",
