...
//! - [`IntervalTiming`] — Sniper-style interval simulation (<10% MAPE, Phase 1)
...
///
/// Full implementation in Phase 1. Currently a stub that delegates to VirtualTiming.
...
impl IntervalTiming {
    pub fn new(ipc: f64, interval_len: u64) -> Self {
        Self { inner: VirtualTiming::new(ipc), interval_len, insns_in_interval: 0 }
    }
}
...
impl Default for AccurateTiming {
    fn default() -> Self { Self { inner: VirtualTiming::new(1.0) } }
}

impl TimingModel for AccurateTiming {
    fn on_insn(&mut self, info: &TimingInsnInfo) -> u64 { self.inner.on_insn(info) }
    fn on_mem_access(&mut self, access: &MemAccess) { self.inner.on_mem_access(access); }
    fn on_branch(&mut self, taken: bool, predicted: bool) { self.inner.on_branch(taken, predicted); }
    fn current_cycles(&self) -> Tick { self.inner.current_cycles() }
