...
mod writer {
    ...
    pub fn emit_config_ini(
        ...
        writeln!(file, "[stats]")?;
        registry.for_each_counter(&mut |path, _v, desc| {
            let _ = writeln!(file, "{path} = {{ type = counter, desc = \"{desc}\" }}");
        });
        registry.for_each_histogram(&mut |path, buckets, desc| {
            let _ = writeln!(
                file,
                "{path} = {{ type = histogram, buckets = {}, desc = \"{desc}\" }}",
                buckets.len()
            );
        });
        registry.for_each_label(&mut |path, _snap, desc| {
            let _ = writeln!(
                file,
                "{path} = {{ type = label_counter, desc = \"{desc}\" }}"
            );
        });
        registry.for_each_formula(&mut |path, _v, desc| {
            let _ = writeln!(file, "{path} = {{ type = formula, desc = \"{desc}\" }}");
        });
        Ok(())
    }
    ...
    mod tests {
        ...
        fn emit_config_ini_creates_file_with_header() {
            ...
            assert!(contents.contains("system.cpu0.cycles = { type = counter"));
        }
