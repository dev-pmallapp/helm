...
Grouped by area: completed items first, then open/future work.
...
#### helm-probe (framework/helm-probe) — ALL DONE
...
#### helm-spy (debug/helm-spy) — DONE
- ~~Wire `ProbePluginBridge` to connect probe events to `HelmSpy`~~ — DONE: `bridge.rs` exists (140 lines, fully tested)
...
#### Advanced Analysis (helm-spy) — ALL DONE
- ~~`SimPoint` BBV computation: `src/analysis/simpoint.rs`~~ — DONE: `BasicBlockVector`, `SimPointCollector` implemented
- ~~`PowerModel`: per-class instruction energy estimation: `src/analysis/power.rs`~~ — DONE: `EnergyTable`, `PowerModel` implemented
...
#### SimObject hierarchy — Phase B (device pyclasses) — DONE
...
- ~~Port wiring support: `device.irq = gic.spi(N)` stores `PortRef` resolved at `instantiate()`~~ — DONE: `PortRef` struct in `src/port.rs` with resolution logic

#### Device introspection — Phase D — DONE
- ~~Device properties read live Rust state after `instantiate()`~~ — DONE: implemented in `src/devices.rs` and `src/system.rs`
- ~~CPU register access through `system.cpu.xn(N)` etc.~~ — DONE: register helpers on System
- ~~GIC/UART state inspection: `system.gic.pending_mask`, `system.uart.tx_count`~~ — DONE: `pending_mask()` and `tx_count` exposed
...
Blocked until helm-spy covers all remaining features.
...
#### helm-engine instrumentation wiring — Phase 3
Partially done (HelmSpy now wired as parallel observation path).
...
- Wire `HelmSpy` fully into `helm-engine` (currently parallel, not sole path)

#### helm-report remaining integration
- Add `helm-report` dep to `helm-python/Cargo.toml` (not yet wired)

#### Open instrumentation design questions
- **DashMap vs sharded counters for HeatMap**: `DashMap` (external dep) vs manual `[Mutex<HashMap>; N]`. Resolve before integration.
...
- **Async delivery thread model**: `AsyncFileSink` background thread — per-sink (simple) or shared global I/O thread pool (fewer threads).
...
Partially done: `MemoryBackend` trait added in `helm-core/src/mem.rs` as the shared contract.
...
#### DLD remaining items (Phase 3+)
- DLD checkpoint migration: optional `helm_device_migrate_checkpoint` C export
...
#### Versioning — remaining items
- **Debug Protocol (HelmProtocol)**: versioned handshake — Phase 3
- **Checkpoint Format**: schema hash for migration — Phase 3
