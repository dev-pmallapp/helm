...
impl BcmGpio {
    ...
    fn handle_read(&self, offset: u64) -> u32 {
        match offset {
            o if o >= GPFSEL0 && o < GPFSEL0 + 24 => self.fsel[((o - GPFSEL0) / 4) as usize],
            o if o >= GPLEV0 && o < GPLEV0 + 8 => self.level[((o - GPLEV0) / 4) as usize],
            o if o >= GPEDS0 && o < GPEDS0 + 8 => self.eds[((o - GPEDS0) / 4) as usize],
            o if o >= GPREN0 && o < GPREN0 + 8 => self.ren[((o - GPREN0) / 4) as usize],
            o if o >= GPFEN0 && o < GPFEN0 + 8 => self.fen[((o - GPFEN0) / 4) as usize],
            o if o >= GPHEN0 && o < GPHEN0 + 8 => self.hen[((o - GPHEN0) / 4) as usize],
            o if o >= GPLEN0 && o < GPLEN0 + 8 => self.len[((o - GPLEN0) / 4) as usize],
            GPPUD => self.pud,
...
impl Device for BcmGpio {
    ...
    fn tick(&mut self, _cycles: u64) -> HelmResult<Vec<DeviceEvent>> {
        ...
        for bank in 0..2 {
            let rising = self.level[bank] & !self.old_level[bank];
            let falling = !self.level[bank] & self.old_level[bank];
            let edge_detected = (rising & self.ren[bank]) | (falling & self.fen[bank]);
            if edge_detected != 0 {
                self.eds[bank] |= edge_detected;
                events.push(DeviceEvent::Irq {
                    line: 49 + bank as u32,
                    assert: true,
                });
            }
            self.old_level[bank] = self.level[bank];
        }
        Ok(events)
    }
}
