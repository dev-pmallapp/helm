...
fn se_progress_10m() {
        ...
        match cpu.step(&mut mem) {
            ...
            Err(helm_core::HelmError::Syscall { number, .. }) => {
                ...
                let result = syscall
                    ...
                    .expect("syscall handler failed");
                cpu.set_xn(0, result);
                if syscall.should_exit {
                    eprintln!(
                        "exit({}) at {insn_count} insns ({sc_count} sc)",
                        syscall.exit_code
                    );
                    return;
                }
    ...
    assert!(
        ...
        "Expected >=1M insns, got {insn_count}"
    );
}
