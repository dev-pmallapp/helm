...
#[test]
fn movz_64_hw0() {
    let (mut c, mut m) = cpu_exec(&[mov_wide(1, 0b10, 0, 0xABCD, 0)]);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xABCD);
}
#[test]
fn movz_64_hw1() {
    let (mut c, mut m) = cpu_exec(&[mov_wide(1, 0b10, 1, 0xABCD, 0)]);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xABCD_0000);
}
#[test]
fn movz_64_hw2() {
    let (mut c, mut m) = cpu_exec(&[mov_wide(1, 0b10, 2, 0xABCD, 0)]);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xABCD_0000_0000);
}
#[test]
fn movz_64_hw3() {
    let (mut c, mut m) = cpu_exec(&[mov_wide(1, 0b10, 3, 0xABCD, 0)]);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xABCD_0000_0000_0000);
}
#[test]
fn movz_32_hw0() {
    let (mut c, mut m) = cpu_exec(&[mov_wide(0, 0b10, 0, 0xFFFF, 0)]);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFFF);
}
#[test]
fn movz_32_hw1() {
    let (mut c, mut m) = cpu_exec(&[mov_wide(0, 0b10, 1, 0xFFFF, 0)]);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFFF_0000);
}
#[test]
fn movn_64_hw0() {
    let (mut c, mut m) = cpu_exec(&[mov_wide(1, 0b00, 0, 0, 0)]);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), u64::MAX);
}
#[test]
fn movn_64_hw1() {
    let (mut c, mut m) = cpu_exec(&[mov_wide(1, 0b00, 1, 0xFFFF, 0)]);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFFF_FFFF_0000_FFFF);
}
#[test]
fn movk_64_hw0() {
    let (mut c, mut m) = cpu_exec(&[mov_wide(1, 0b11, 0, 0x1234, 0)]);
    c.set_xn(0, 0xFFFF_FFFF_FFFF_FFFF);
    c.step(&mut m).unwrap();
...
#[test]
fn ubfm_lsr_64_by1() {
    let (mut c, mut m) = cpu_exec(&[bitfield(1, 0b10, 1, 1, 63, 1, 0)]);
    c.set_xn(1, 0xF0);
    c.step(&mut m).unwrap();
...
#[test]
fn ubfm_lsr_64_by32() {
    let (mut c, mut m) = cpu_exec(&[bitfield(1, 0b10, 1, 32, 63, 1, 0)]);
    c.set_xn(1, 0x1_0000_0000);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 1);
}
#[test]
fn ubfm_lsl_64_by8() {
    let (mut c, mut m) = cpu_exec(&[bitfield(1, 0b10, 1, 56, 55, 1, 0)]);
    c.set_xn(1, 0xFF);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFF00);
}
#[test]
fn ubfm_uxtb_32() {
    let (mut c, mut m) = cpu_exec(&[bitfield(0, 0b10, 0, 0, 7, 1, 0)]);
    c.set_xn(1, 0x1234_56FF);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFF);
}
#[test]
fn sbfm_sxtb_neg() {
    let (mut c, mut m) = cpu_exec(&[bitfield(1, 0b00, 1, 0, 7, 1, 0)]);
    c.set_xn(1, 0x80);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFFF_FFFF_FFFF_FF80);
}
#[test]
fn sbfm_sxtb_pos() {
    let (mut c, mut m) = cpu_exec(&[bitfield(1, 0b00, 1, 0, 7, 1, 0)]);
...
#[test]
fn sbfm_sxth_neg() {
    let (mut c, mut m) = cpu_exec(&[bitfield(1, 0b00, 1, 0, 15, 1, 0)]);
    c.set_xn(1, 0x8000);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFFF_FFFF_FFFF_8000);
}
#[test]
fn sbfm_sxtw_neg() {
    let (mut c, mut m) = cpu_exec(&[bitfield(1, 0b00, 1, 0, 31, 1, 0)]);
    c.set_xn(1, 0x8000_0000);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFFF_FFFF_8000_0000);
}
#[test]
fn bfm_bfi_low8() {
    let (mut c, mut m) = cpu_exec(&[bitfield(0, 0b01, 0, 0, 7, 1, 0)]);
    c.set_xn(0, 0xFF00);
    c.set_xn(1, 0xAB);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFAB);
}
...
#[test]
fn add_reg_64_lsl0() {
    let (mut c, mut m) = cpu_exec(&[add_sub_reg(1, 0, 0, 0b00, 2, 0, 1, 0)]);
    c.set_xn(1, 10);
    c.set_xn(2, 3);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 13);
...
#[test]
fn add_reg_64_lsl4() {
    let (mut c, mut m) = cpu_exec(&[add_sub_reg(1, 0, 0, 0b00, 2, 4, 1, 0)]);
    c.set_xn(1, 10);
    c.set_xn(2, 1);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 26);
...
#[test]
fn add_reg_64_lsl8() {
    let (mut c, mut m) = cpu_exec(&[add_sub_reg(1, 0, 0, 0b00, 2, 8, 1, 0)]);
    c.set_xn(1, 0);
    c.set_xn(2, 1);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 256);
}
#[test]
fn add_reg_64_lsr4() {
    let (mut c, mut m) = cpu_exec(&[add_sub_reg(1, 0, 0, 0b01, 2, 4, 1, 0)]);
    c.set_xn(1, 10);
    c.set_xn(2, 0x100);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 26);
...
#[test]
fn add_reg_64_lsr8() {
    let (mut c, mut m) = cpu_exec(&[add_sub_reg(1, 0, 0, 0b01, 2, 8, 1, 0)]);
    c.set_xn(1, 0);
    c.set_xn(2, 0x100);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 1);
...
#[test]
fn add_reg_64_asr4() {
    let (mut c, mut m) = cpu_exec(&[add_sub_reg(1, 0, 0, 0b10, 2, 4, 1, 0)]);
    c.set_xn(1, 100);
    c.set_xn(2, (-32i64) as u64);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 98);
}
#[test]
fn sub_reg_64_lsl4() {
    let (mut c, mut m) = cpu_exec(&[add_sub_reg(1, 1, 0, 0b00, 2, 4, 1, 0)]);
    c.set_xn(1, 100);
    c.set_xn(2, 2);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 68);
...
#[test]
fn add_reg_32_lsl4() {
    let (mut c, mut m) = cpu_exec(&[add_sub_reg(0, 0, 0, 0b00, 2, 4, 1, 0)]);
    c.set_xn(1, 10);
    c.set_xn(2, 1);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 26);
...
#[test]
fn sub_reg_32_lsl4() {
    let (mut c, mut m) = cpu_exec(&[add_sub_reg(0, 1, 0, 0b00, 2, 4, 1, 0)]);
    c.set_xn(1, 100);
    c.set_xn(2, 2);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 68);
...
#[test]
fn adds_reg_64_lsl4() {
    let (mut c, mut m) = cpu_exec(&[add_sub_reg(1, 0, 1, 0b00, 2, 4, 1, 31)]);
    c.set_xn(1, u64::MAX - 31);
    c.set_xn(2, 2);
    c.step(&mut m).unwrap();
    assert!(c.regs.z());
}
#[test]
fn subs_reg_64_lsr4() {
    let (mut c, mut m) = cpu_exec(&[add_sub_reg(1, 1, 1, 0b01, 2, 4, 1, 31)]);
    c.set_xn(1, 16);
    c.set_xn(2, 0x100);
    c.step(&mut m).unwrap();
    assert!(c.regs.z());
...
#[test]
fn and_reg_64_lsl4() {
    let (mut c, mut m) = cpu_exec(&[log_reg(1, 0b00, 0, 0b00, 2, 4, 1, 0)]);
    c.set_xn(1, 0xFFF0);
    c.set_xn(2, 0xFF);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFF0);
}
#[test]
fn orr_reg_64_lsl8() {
    let (mut c, mut m) = cpu_exec(&[log_reg(1, 0b01, 0, 0b00, 2, 8, 1, 0)]);
    c.set_xn(1, 0xFF);
    c.set_xn(2, 0xFF);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFFF);
...
#[test]
fn eor_reg_64_lsr4() {
    let (mut c, mut m) = cpu_exec(&[log_reg(1, 0b10, 0, 0b01, 2, 4, 1, 0)]);
    c.set_xn(1, 0x0F);
    c.set_xn(2, 0xF0);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0);
...
#[test]
fn bic_reg_64_lsl0() {
    let (mut c, mut m) = cpu_exec(&[log_reg(1, 0b00, 1, 0b00, 2, 0, 1, 0)]);
    c.set_xn(1, 0xFF);
    c.set_xn(2, 0x0F);
    c.step(&mut m).unwrap();
...
#[test]
fn orn_reg_64_lsl0() {
    let (mut c, mut m) = cpu_exec(&[log_reg(1, 0b01, 1, 0b00, 2, 0, 1, 0)]);
    c.set_xn(1, 0);
    c.set_xn(2, 0xFF);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), !0xFFu64);
}
#[test]
fn eon_reg_64_lsl0() {
    let (mut c, mut m) = cpu_exec(&[log_reg(1, 0b10, 1, 0b00, 2, 0, 1, 0)]);
    c.set_xn(1, 0xAA);
    c.set_xn(2, 0xFF);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), !(0xAA ^ 0xFF));
}
#[test]
fn ands_reg_64_lsl0() {
    let (mut c, mut m) = cpu_exec(&[log_reg(1, 0b11, 0, 0b00, 2, 0, 1, 31)]);
    c.set_xn(1, 0xFF);
    c.set_xn(2, 0x100);
    c.step(&mut m).unwrap();
    assert!(c.regs.z());
...
#[test]
fn bics_reg_64_lsl0() {
    let (mut c, mut m) = cpu_exec(&[log_reg(1, 0b11, 1, 0b00, 2, 0, 1, 31)]);
    c.set_xn(1, 0xFF);
    c.set_xn(2, 0xFF);
    c.step(&mut m).unwrap();
    assert!(c.regs.z());
...
#[test]
fn and_reg_32_lsl4() {
    let (mut c, mut m) = cpu_exec(&[log_reg(0, 0b00, 0, 0b00, 2, 4, 1, 0)]);
    c.set_xn(1, 0xFFF0);
    c.set_xn(2, 0xFF);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFF0);
...
#[test]
fn orr_reg_32_lsl8() {
    let (mut c, mut m) = cpu_exec(&[log_reg(0, 0b01, 0, 0b00, 2, 8, 1, 0)]);
    c.set_xn(1, 0xFF);
    c.set_xn(2, 0xFF);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFFF);
...
#[test]
fn ubfm_lsr_32_by1() {
    let (mut c, mut m) = cpu_exec(&[bitfield(0, 0b10, 0, 1, 31, 1, 0)]);
    c.set_xn(1, 0xFF);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0x7F);
}
#[test]
fn ubfm_lsr_32_by16() {
    let (mut c, mut m) = cpu_exec(&[bitfield(0, 0b10, 0, 16, 31, 1, 0)]);
    c.set_xn(1, 0xFFFF_0000);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFFF);
}
#[test]
fn ubfm_lsr_64_by4() {
    let (mut c, mut m) = cpu_exec(&[bitfield(1, 0b10, 1, 4, 63, 1, 0)]);
    c.set_xn(1, 0xF0);
    c.step(&mut m).unwrap();
...
#[test]
fn ubfm_lsr_64_by48() {
    let (mut c, mut m) = cpu_exec(&[bitfield(1, 0b10, 1, 48, 63, 1, 0)]);
    c.set_xn(1, 0xFFFF_0000_0000_0000);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFFF);
}
#[test]
fn ubfm_lsl_32_by1() {
    let (mut c, mut m) = cpu_exec(&[bitfield(0, 0b10, 0, 31, 30, 1, 0)]);
    c.set_xn(1, 0x7F);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFE);
}
#[test]
fn ubfm_lsl_32_by8() {
    let (mut c, mut m) = cpu_exec(&[bitfield(0, 0b10, 0, 24, 23, 1, 0)]);
    c.set_xn(1, 0xFF);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFF00);
}
#[test]
fn ubfm_lsl_64_by1() {
    let (mut c, mut m) = cpu_exec(&[bitfield(1, 0b10, 1, 63, 62, 1, 0)]);
    c.set_xn(1, 0x7F);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFE);
}
#[test]
fn ubfm_lsl_64_by32() {
    let (mut c, mut m) = cpu_exec(&[bitfield(1, 0b10, 1, 32, 31, 1, 0)]);
    c.set_xn(1, 0xFFFF_FFFF);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFFF_FFFF_0000_0000);
}
#[test]
fn sbfm_asr_64_by1() {
    let (mut c, mut m) = cpu_exec(&[bitfield(1, 0b00, 1, 1, 63, 1, 0)]);
    c.set_xn(1, 0x8000_0000_0000_0000);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xC000_0000_0000_0000);
}
#[test]
fn sbfm_asr_64_by32() {
    let (mut c, mut m) = cpu_exec(&[bitfield(1, 0b00, 1, 32, 63, 1, 0)]);
    c.set_xn(1, 0x8000_0000_0000_0000);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFFF_FFFF_8000_0000);
}
#[test]
fn sbfm_asr_32_by1() {
    let (mut c, mut m) = cpu_exec(&[bitfield(0, 0b00, 0, 1, 31, 1, 0)]);
    c.set_xn(1, 0x8000_0000);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xC000_0000);
}
#[test]
fn sbfm_asr_32_by16() {
    let (mut c, mut m) = cpu_exec(&[bitfield(0, 0b00, 0, 16, 31, 1, 0)]);
    c.set_xn(1, 0x8000_0000);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFFF_8000);
}
...
#[test]
fn and_imm_64_low32() {
    let (mut c, mut m) = cpu_exec(&[log_imm(1, 0b00, 1, 0, 31, 1, 0)]);
    c.set_xn(1, u64::MAX);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFFF_FFFF);
}
#[test]
fn and_imm_64_high32() {
    let (mut c, mut m) = cpu_exec(&[log_imm(1, 0b00, 1, 32, 31, 1, 0)]);
    c.set_xn(1, u64::MAX);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFFF_FFFF_0000_0000);
}
#[test]
fn orr_imm_64_bit0() {
    let (mut c, mut m) = cpu_exec(&[log_imm(1, 0b01, 1, 0, 0, 1, 0)]);
    c.set_xn(1, 0);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 1);
}
#[test]
fn eor_imm_64_low8() {
    let (mut c, mut m) = cpu_exec(&[log_imm(1, 0b10, 1, 0, 7, 1, 0)]);
    c.set_xn(1, 0xAA);
    c.step(&mut m).unwrap();
...
#[test]
fn ands_imm_64_z() {
    let (mut c, mut m) = cpu_exec(&[log_imm(1, 0b11, 1, 0, 7, 1, 31)]);
    c.set_xn(1, 0x100);
    c.step(&mut m).unwrap();
    assert!(c.regs.z());
}
...
#[test]
fn and_imm_32_low8() {
    let (mut c, mut m) = cpu_exec(&[log_imm(0, 0b00, 0, 0, 7, 1, 0)]);
    c.set_xn(1, 0xABCD);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xCD);
}
#[test]
fn orr_imm_32_high16() {
    let (mut c, mut m) = cpu_exec(&[log_imm(0, 0b01, 0, 16, 15, 1, 0)]);
    c.set_xn(1, 0xFF);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFFF_00FF);
}
...
#[test]
fn movz_clears_prev() {
    let (mut c, mut m) = cpu_exec(&[mov_wide(1, 0b10, 0, 0, 0)]);
    c.set_xn(0, u64::MAX);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0);
}
...
fn movk_hw1_preserve() {
    let (mut c, mut m) = cpu_exec(&[mov_wide(1, 0b11, 1, 0xABCD, 0)]);
    c.set_xn(0, 0x1234_5678_9ABC_DEF0);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0x1234_5678_ABCD_DEF0);
}
#[test]
fn movk_hw2_preserve() {
    let (mut c, mut m) = cpu_exec(&[mov_wide(1, 0b11, 2, 0x1111, 0)]);
    c.set_xn(0, 0xFFFF_FFFF_FFFF_FFFF);
    c.step(&mut m).unwrap();
...
fn movn_32_1() {
    ...
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0) & 0xFFFF_FFFF, 0xFFFF_FFFE);
}
#[test]
fn movn_64_hw2() {
    let (mut c, mut m) = cpu_exec(&[mov_wide(1, 0b00, 2, 0xFFFF, 0)]);
    c.step(&mut m).unwrap();
    assert_eq!(c.xn(0), 0xFFFF_0000_FFFF_FFFF);
}
